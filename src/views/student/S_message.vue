  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="身份证号">
          <el-input v-model="editStu.ID_number" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="院系专业">
          <el-cascader v-model="value" :options="options"></el-cascader>
        </el-form-item>
        <el-form-item label="年级">
          <el-input v-model="editStu.class" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  data() {
    return {
      ...
      array1: ["学号", "姓名", "院系", "专业", "联系方式"],
      array2: ["account", "name", "department", "major", "phone"],
      // 分页
      queryInfo: {
        quwey: "",
        pagenum: 1,
  ...
  methods: {
    // 查询信息
    async selectmessage() {
      const { data: res } = await this.$http.get("/student/message");
    ...
    async removeMore() {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(async () => {
          ...
          const { data: res } = await this.$http.post("/del_messagem", arr);
          if (res.status != 201) {
            return this.$message.error("删除失败");
          }
          this.$message.success("删除成功");
