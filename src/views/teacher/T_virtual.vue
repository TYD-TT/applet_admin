  ...
  <div>
    <div>
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>虚拟服务器</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
...
export default {
  data() {
    return {
      ...
      array1: ["工号", "姓名","联系方式", "地点","部门类型","所属部门", "申请日期"],
      array2: ["account", "name", "phone", "address","section_type","section", "creat_time"],
      // 分页
      queryInfo: {
        quwey: "",
        pagenum: 1,
  ...
  methods: {
    // 查询信息
    async selectmessage() {
      const { data: res } = await this.$http.get("/teacher/virtual");
    ...
    async removeMore() {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(async () => {
          ...
          const { data: res } = await this.$http.post("/teacher/del_virtual", arr);
          if (res.status != 201) {
            return this.$message.error("删除失败");
          }
          this.$message.success("删除成功");
    ...
    exportToExcel() {
      require.ensure([], () => {
        ...
        const tHeader = ["工号", "姓名","联系方式", "地点","部门类型","所属部门","CPU","内存","硬盘","预装系统", "申请日期"];
        const filterVal = [
          "account",
          "name",
          "phone",
    ...
    async select_studentMessage() {
      const { data: res } = await this.$http.post("/teacher/find_virtual", this.searchList);
      this.tableData = [];
      this.tableData = JSON.parse(res.data);
      this.total = this.tableData.length;
      this.searchList = {};
