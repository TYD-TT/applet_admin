  ...
  <div>
    <div>
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>端口开通</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    ...
    <el-card>
      ...
      >
        ...
        <el-table-column prop="address" label="开通端口办公地" width="120">
        </el-table-column>
        <el-table-column prop="section_type" label="部门类型" width="120">
        </el-table-column>
        <el-table-column prop="section" label="所在部门"> </el-table-column>
        <el-table-column prop="port_number" label="端口数量" width="120">
        </el-table-column>
        <el-table-column prop="port_use" label="端口用途" width="100">
        </el-table-column>
    ...
    >
      ...
      >
        ...
        <el-form-item label="端口数量">
          <el-input v-model="editStu.port_number" autocomplete="off" disabled></el-input>
        </el-form-item>
        <el-form-item label="端口用途">
          <el-input
            type="textarea"
            :rows="2"
            v-model="editStu.port_use"
            disabled
          ></el-input>
        </el-form-item>
...
export default {
  data() {
    return {
      ...
      array2: ["account", "name",  "phone", "section", "creat_time"],
      // 分页
      queryInfo: {
        quwey: "",
        pagenum: 1,
  ...
  methods: {
    // 查询信息
    async selectmessage() {
      const { data: res } = await this.$http.get("/teacher/port");
    ...
    async removeMore() {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(async () => {
          ...
          const { data: res } = await this.$http.post("/teacher/del_port", arr);
          if (res.status != 201) {
            return this.$message.error("删除失败");
          }
          this.$message.success("删除成功");
    ...
    async select_studentMessage() {
      const { data: res } = await this.$http.post("/teacher/find_port", this.searchList);
      this.tableData = [];
      this.tableData = JSON.parse(res.data);
      this.total = this.tableData.length;
      this.searchList = {};
